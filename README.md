...
- **Synchronization**  
  - A simple **spinlock** abstraction (`spinlock_t`) built from atomic test-and-set, used to protect the in-memory filesystem.

- **Protection**  
  - Software-level protection via **per-file owner + permission flags**:
    - `owner` (task id or `-1` for public)
    - `perm` bits: `1 = read`, `2 = write`  
  - Tasks can’t open or modify files they don’t have permission for.
...
- **Anything else you can think of**  
  - Minimal UART driver to print to QEMU console.  
  - Simple `uart_printf()` for debugging.  
  - Clean separation of `boot/`, `kernel/`, and `user/` code.
